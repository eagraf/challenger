...
import (
	...
	"github.com/eagraf/challenger"
	"github.com/google/uuid"
	"github.com/joho/godotenv"
)

type TestServer struct {
	challengePersister challenger.ChallengeSessionPersister
...
func (s *TestServer) challengeHandler(w http.ResponseWriter, r *http.Request) {
	...
	session := &challenger.ChallengeSession{
		SessionID: sessionID,
		Challenge: challenge,
...
func (s *TestServer) authHandler(w http.ResponseWriter, r *http.Request) {
	...
	// Verify proof
	valid, err := challenger.VerifyProof(session.Challenge, req.Proof, s.publicKey)
	if err != nil || !valid {
	...
	// Generate JWT
	token, err := challenger.GenerateJWT(s.signingKey)
	if err != nil {
...
func (s *TestServer) HelloWorldAuthenticatedHandler(w http.ResponseWriter, r *http.Request) {
	...
	err := challenger.ValidateRequest(r, s.signingKey)
	if err != nil {
...
func main() {
	...
	sessionPersister := challenger.NewInMemorySessionPersister()
